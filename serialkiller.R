...
mu <- mean(gang$AgeFirstKill)
sigma <- sd(gang$AgeFirstKill)


z <- (sort(gang$AgeFirstKill) - mu)/sigma

n <- length(gang$AgeFirstKill)
r <- (1:n)
...
# Normal Q-Q plot:
plot(q, z, 
     main="Q-Q plot ( Gang or organized crime )", 
...
#Plotting the confidence intervals using Forest plot
